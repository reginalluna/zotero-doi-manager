...
async function updateItems(items, operation, { silent = false } = {}) {
  if (isRunning) return false;

  const { supported, unsupported } = partitionItems(items);
  if (!silent && unsupported.length > 0) showUnsupportedWarning(unsupported);
  if (supported.length === 0) return true;

  isRunning = true;
  const prefs = readPrefs();
  const counts = { updated: 0, invalid: 0, nodoi: 0, multiple: 0, skipped: 0 };
  const progressWindow = silent ? null : openProgressWindow(operation);

  try {
    for (let i = 0; i < supported.length; i++) {
      if (progressWindow) updateProgress(progressWindow, i + 1, supported.length);
      const outcome = await processItem(supported[i], operation, prefs);
      counts[outcome] = (counts[outcome] ?? 0) + 1;
    }
    if (!silent) showCompletion(progressWindow, operation, { counts }, prefs);
    return true;
  } catch (error) {
